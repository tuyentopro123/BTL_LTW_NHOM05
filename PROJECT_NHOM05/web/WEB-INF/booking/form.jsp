...
<div class="content">
    <form action="/bookings" id="form" <c:if test="${booking == null}">method="POST" </c:if> >
        <h1><c:choose><c:when test="${booking == null}">Đặt</c:when><c:otherwise>Edit</c:otherwise></c:choose> phòng</h1>
        <label for="arrival">Bắt đầu</label>
        <input type="date" name="arrival" id="arrival" autocomplete="off" min="${minDateArrival}" value="${booking.getArrival()}" required>
        <label for="departure">Kết thúc</label>
        ...
        <div class="split">
            ...
            <label for="id_room">Số phòng</label>
        ...
        <label for="id_guest">Khách hàng</label>
        ...
        <div class="submit">
            <button onclick="window.history.go(-1);" type="button">Trờ lại</button>
        ...
        if(document.getElementById("room_type").value !== ""){
            ...
            fetch(url, {
                ...
                .then(data => {
                    ...
                    document.getElementById("result").innerHTML = formatNumberWithDot(total) + " VNĐ";
                });
        }
    }
